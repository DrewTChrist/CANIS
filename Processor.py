...
class ImageProcessor:
    ...
    def process(self, save_processed_img=False):
        # The image is first converted to grayscale which replaces each pixel's RGB values with just a single
        # brightness value
        self.img = self.img.convert('L')

        # For each pixel, if the brightness value is below the brightness threshold then set the pixel to black,
    ...
    def extract_vertices(self):
        # Extract vertices from the image by searching for white pixels. If a white pixel is within 5 pixels of another
        # white pixel, then the pixel is treated as a duplicate and the star is not counted twice
